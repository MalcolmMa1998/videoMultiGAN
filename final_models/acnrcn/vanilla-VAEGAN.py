...
class VAEGAN():
    ...
    def __init__(self, batch_size=16, image_shape=[28, 28, 3], embedding_size=128,
                 ...
                 num_class_image=13, frames=2, frames_input=2, total_size=16000, video_create=False):
    ...
    def generate_batch(self):
        ...
        return (self.x[self.iter:self.iter + self.batch_size], self.x_old[self.iter:self.iter + batch_size],
                self.x_gen[self.iter:self.iter + batch_size],
...
def train_epoch(gan, placeholders, flag=False, initial=True):
    ...
    while run <= num_examples:
        for t in range(final_iter):
            ...
            if initial:
                _, _, loss_val[3], loss_val[0] = session.run([
                    optimizers["text_discriminator"], optimizers["discriminator"],
                    losses["disc_text_classifier"], losses["disc_image_discriminator"]
                ], feed_dict=feed_dict)
            else:
        ...
        for _ in range(2 * diter):
            ...
            feed_dict = get_feed_dict(gan, placeholders)
            if initial:
                _, _, _, loss_val[6], loss_val[4], loss_val[5] = session.run([
                    optimizers["generator"], optimizers["encoder"],
                    optimizers["text_encoder"],
...
save_visualization(np.concatenate([image_sample, image_gen], axis=3),
                   save_path='../../results/acrcn/32/%s/sample.jpg' % (sys.argv[-2]))
...
print("Running code: ")
saver.restore(session, "/extra_data/prannay/trained_models/just_vanilla.ckpt")
...
for e in range(epoch):
    ...
    save_visualization(np.concatenate([image_sample, images], axis=3),
                       save_path="../../results/acrcn/32/%s/sample_%d.jpg" % (sys.argv[-2], ep + 1))
