...
def rot_generator(batch_size, frames, frames_input=3):
    ...
    batch1in = np.reshape(batch1in, [-1, 28 * 28])
...
def rot_text_generator(batch_size, frames, frames_input=3):
    ...
    for i in range(batch_size):
        ...
        rot = np.random.normal(0, 5)
        if t == 0:
            sentence = "the digit %s is moving to the left downwards while it rotates %s" % (
                sentence_proc(batch1_labels[i], rot))
            ...
            sentence = "the digit %s is moving to the right downwards while it rotates %s" % (
                sentence_proc(batch1_labels[i], rot))
            text_labels[i] = convert_embedding(sentence)
            # text_labels[i] = np.array([rot, 1,-1,-1,1])
            # text_labels[i][-1] *= random
            ...
            sentence = "the digit %s is moving to the right upwards while it rotates %s" % (
                sentence_proc(batch1_labels[i], rot))
            text_labels[i] = convert_embedding(sentence)
            # text_labels[i] = np.array([rot, -1,-1,1,1])
            # text_labels[i][-1] *= random
...
def text_generator(batch_size, frames):
    ...
    for i in range(batch_size):
        ...
        if t == 0:
            ...
            for r in range(frames):
                for j in range(3):
                    batch_gen[i, 10 + (random * r):38 + (random * r), 10 + (random * r):38 + (random * r),
                    j + (3 * r)] = batch1[i] * l[j]
        elif t == 1:
            # text_labels[i] = np.array([1,-1,-1,1])
            # text_labels[i][-1] *= random
            ...
            for r in range(frames):
                for j in range(3):
                    batch_gen[i, 26 - (random * r):54 - (random * r), 26 - (random * r):54 - (random * r),
                    j + (3 * r)] = batch1[i] * l[j]
        elif t == 2:
            # text_labels[i] = np.array([-1,-1,1,1])
            # text_labels[i][-1] *= random
            ...
            for r in range(frames):
                for j in range(3):
                    batch_gen[i, 26 - (random * r):54 - (random * r), 10 + (random * r):38 + (random * r),
                    j + (3 * r)] = batch1[i] * l[j]
        else:
            # text_labels[i] = np.array([1,1,-1,-1])
            # text_labels[i][-1] *= random
            ...
            for r in range(frames):
                for j in range(3):
                    batch_gen[i, 10 + (random * r):38 + (random * r), 26 - (random * r):54 - (random * r),
                    j + (3 * r)] = batch1[i] * l[j]
