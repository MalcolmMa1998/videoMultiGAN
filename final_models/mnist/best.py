...
from tensorflow_core.python.framework.test_ops import list_output

from final_models.mnist.generator import rot_generator as generate
...
class VAEGAN():
    ...
    def gan_loss(self, X, Y, discriminator, X_in, Y_in, reuse=False, name=None,
                 scope=tf.variable_scope("random_testing"), flag=True):
        if not flag:
    ...
    def generate_batch(self):
        ...
        return (self.x[self.iter:self.iter + self.batch_size], self.x_old[self.iter:self.iter + self.batch_size],
                self.x_gen[self.iter:self.iter + self.batch_size],
    ...
    def create_frames(self, image_input, x, z_s, z_c, image_class_input, text_label_input, z_t):
        ...
        with tf.variable_scope("transformation") as scope:
            if not self.first_time:
                scope.reuse_variables()
            encoder_alt = tf.layers.dense(encode[:, :self.embedding_size], units=int(encode.shape[-1]),
                                          reuse=scope.reuse,
        ...
        with tf.variable_scope("image_classifier") as scope:
            ...
            D_z_c_loss = self.gan_loss(D_z_hat_c, D_z_real, self.discriminate_encode, z_hat_c, image_class_input,
                                       scope=scope)
            D_z_c_loss += self.gan_loss(D_z_c, D_z_real, self.discriminate_encode, z_c, image_class_input, scope=scope)
            G_z_c_loss = self.gan_loss(D_z_hat_c, D_z_real, self.discriminate_encode, z_hat_c, image_class_input,
                                       scope=scope, flag=True)
            G_z_c_loss += self.gan_loss(D_z_c, D_z_real, self.discriminate_encode, z_c, image_class_input, scope=scope,
                                        flag=True)
        with tf.variable_scope("style_classifier") as scope:
            ...
            D_z_s_loss += self.gan_loss(D_z_hat_s_fut, D_z_s, self.discriminate_encode, z_hat_s_fut, z_hat_s,
                                        scope=scope)
            G_z_s_loss = self.gan_loss(D_z_hat_s, D_z_s, self.discriminate_encode, z_hat_s, z_hat_s, scope=scope,
                                       flag=True)
            G_z_s_loss += self.gan_loss(D_z_hat_s_fut, D_z_s, self.discriminate_encode, z_hat_s_fut, z_hat_s,
                                        scope=scope, flag=True)
    ...
    def build_model(self):
        ...
        for i in range(self.frames):
            list_return = self.create_frames(next_image_input,
                                             x[:, :, :, self.dim_channel * i:self.dim_channel * i + self.dim_channel],
                                             z_s[self.batch_size * i:self.batch_size * i + self.batch_size],
                                             z_c[self.batch_size * i:self.batch_size * i + self.batch_size],
                                             image_class_input, text_label_input,
                                             z_t[self.batch_size * i:self.batch_size * i + self.batch_size])
            for count, item in enumerate(list_return):
                list_values[count].append(item)
            if self.video_create:
                next_image_input = tf.concat(axis=3, values=[next_image_input[:, :, :, self.dim_channel:], x[:, :, :,
                                                                                                           self.dim_channel * i:self.dim_channel * i + self.dim_channel]])
            else:
                next_image_input = tf.concat(axis=3,
                                             values=[next_image_input[:, :, :, self.dim_channel:], list_output[0]])
        ...
        with tf.variable_scope("losses"):
            losses["reconstruction"] = tf.sqrt(tf.reduce_mean(tf.square(x - x_hat_fut))) + tf.sqrt(
                tf.reduce_mean(tf.square(x_old - x_hat)))
            ...
            losses["disc_image_discriminator"] = D_x_loss * self.gan_scale + (
                        self.lambda_2 * losses["anti-reconstruction"])
            ...
            losses["encoder"] = losses["gen_image_classifier"] + (self.lambda_1 * losses["reconstruction"]) + losses[
                "gen_style_classifier"] - (self.lambda_2 * losses["anti-reconstruction"])
            losses["transformation"] = -losses["anti-reconstruction"] * self.lambda_2 + self.lambda_1 * losses[
                "reconstruction"]
        ...
        variable_dict["transformation"] = [i for i in filter(lambda x: x.name.startswith("transformation"),
                                                             tf.trainable_variables())]
        variable_dict["text_encoder"] = [i for i in
                                         filter(lambda x: x.name.startswith("text_encoder"), tf.trainable_variables())]
        variable_dict["generator"] = [i for i in
                                      filter(lambda x: x.name.startswith("generator"), tf.trainable_variables())]
        variable_dict["image_disc"] = [i for i in
                                       filter(lambda x: x.name.startswith("image_disc"), tf.trainable_variables())]
        variable_dict["image_class"] = [i for i in
                                        filter(lambda x: x.name.startswith("image_class"), tf.trainable_variables())]
        variable_dict["text_class"] = [i for i in
                                       filter(lambda x: x.name.startswith("text_class"), tf.trainable_variables())]
        variable_dict["style_class"] = [i for i in
                                        filter(lambda x: x.name.startswith("style_class"), tf.trainable_variables())]
        print("Completed weights")
        optimizer = dict()
        with tf.variable_scope("optimizers"):
            # encoder_adam = tf.train.AdamOptimizer(self.learning_rate[0],beta1=0.5,beta2=0.9)
            print("encoder")
            optimizer["encoder"] = tf.train.AdamOptimizer(self.learning_rate[0], beta1=0.5, beta2=0.9).minimize(
                losses["encoder"], var_list=variable_dict["encoder"])
            optimizer["transformation"] = tf.train.AdamOptimizer(self.learning_rate[0], beta1=0.5, beta2=0.9).minimize(
                losses["transformation"], var_list=variable_dict["transformation"])
            print("text_encoder")
            optimizer["text_encoder"] = tf.train.AdamOptimizer(self.learning_rate[1], beta1=0.5, beta2=0.9).minimize(
                losses["text_encoder"], var_list=variable_dict["text_encoder"])
            print("generator")
            optimizer["generator"] = tf.train.AdamOptimizer(self.learning_rate[0], beta1=0.5, beta2=0.9).minimize(
                losses["generator_image"], var_list=variable_dict["generator"])
            optimizer["generator_gan"] = tf.train.AdamOptimizer(self.learning_rate[0], beta1=0.5, beta2=0.9).minimize(
                losses["generator_image_gan"], var_list=variable_dict["generator"])
            optimizer["generator_reconstruction"] = tf.train.AdamOptimizer(self.learning_rate[0], beta1=0.5,
                                                                           beta2=0.9).minimize(
                self.lambda_1 * losses["reconstruction"], var_list=variable_dict["generator"])
            print("disc_image")
            optimizer["discriminator"] = tf.train.AdamOptimizer(self.learning_rate[3], beta1=0.5, beta2=0.9).minimize(
                losses["disc_image_discriminator"], var_list=variable_dict["image_disc"])
            print("disc_non_image")
            optimizer["code_discriminator"] = tf.train.AdamOptimizer(self.learning_rate[4], beta1=0.5,
                                                                     beta2=0.9).minimize(
                losses["disc_image_classifier"], var_list=variable_dict["image_class"])
            optimizer["text_discriminator"] = tf.train.AdamOptimizer(self.learning_rate[5], beta1=0.5,
                                                                     beta2=0.9).minimize(losses["disc_text_classifier"],
                                                                                         var_list=variable_dict[
                                                                                             "text_class"])
            optimizer["style_discriminator"] = tf.train.AdamOptimizer(self.learning_rate[6], beta1=0.5,
                                                                      beta2=0.9).minimize(
                losses["disc_style_classifier"], var_list=variable_dict["style_class"])
...
def get_feed_dict(gan, placeholders):
    ...
    feed_dict = {
        ...
        placeholders['z_t']: np.concatenate(
            [np.random.normal(0, 1, [batch_size * frames, num_class_motion]), frame_label(batch_size, frames)], axis=1)
    }
    return feed_dict
...
def train_epoch(gan, placeholders, flag=False, initial=True):
    ...
    while run <= num_examples:
        ...
        if count % 10 == 0 or flag:
            print("%d:%d : " % (ep + 1, run) + " : ".join(map(lambda x: str(x), loss_val)) + " " + str(
...
save_visualization(np.concatenate([image_sample, image_old], axis=3),
                   save_path='../results/final/mnist64/%s/sample.jpg' % (sys.argv[-2]))
save_visualization(np.concatenate([image_sample, image_gen], axis=3),
                   save_path='../results/final/mnist64/%s/sample00.jpg' % (sys.argv[-2]))
...
for e in range(epoch):
    ...
    save_visualization(np.concatenate([image_sample, images], axis=3),
                       save_path="../results/final/mnist64/%s/sample_%d.jpg" % (sys.argv[-2], ep + 1))
    summary = session.run(merged, feed_dict=feed_dict)
    train_writer.add_summary(summary, ep)
    saver.save(session, "/extra_data/prannay/trained_models/mnist_53_model_5.ckpt")
